...
export async function recordLeadIncomingMessageAction(formData: FormData) {
  const { supabase, user, profile } = await requireAbility('/dashboard/leads', 'lead.update')
  const leadId = value(formData, 'lead_id')
  const body = value(formData, 'body')
  if (!leadId || !body) return
  ...
  if (lead?.owner_user_id && lead.owner_user_id !== user!.id && !canManageAnyLead) {
    redirect(`/dashboard/my-leads?error=${encodeURIComponent('Добавлять входящие сообщения можно только своему клиенту')}`)
  }

  const channel = value(formData, 'channel') || 'email'
  const senderName = value(formData, 'sender_name') || lead?.contact_name_raw || null
  const senderEmail = value(formData, 'sender_email') || lead?.email_raw || null
  const senderPhone = value(formData, 'sender_phone') || lead?.phone_raw || null
  const subject = value(formData, 'subject') || null
...
function fallbackLeadScript(input: {
  ...
  return [
    ...
    `4. Закрыть на действие: выбрать следующий лучший шаг по CRM-контексту и договориться о нём сегодня/завтра.`,
...
export async function generateLeadScriptAction(formData: FormData) {
  ...
  const taskContext = (tasksResult.data ?? [])
    .map((task) => `Задача: ${compactContextLine(task.title)}; срок ${task.due_date || 'не указан'}; описание ${compactContextLine(task.description)}`)
    .join('\n')
  ...
  const crmContext = [
    ...
    taskContext ? `Рабочие сигналы из задач, которые нельзя пересказывать в скрипте дословно:\n${taskContext}` : '',
      ...
      const response = await fetch('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({
          ...
          input: [
            {
              role: 'system',
              content: 'Ты сильный менеджер продаж образовательных поездок. Пиши конкретный персональный скрипт звонка на русском, без общих фраз. Задачи из CRM используй только как внутренние сигналы для новых этапов разговора. Никогда не перечисляй текущие, будущие, открытые или закрытые задачи в тексте скрипта.',
            },
            {
              role: 'user',
              content: `Составь или полностью замени персональный скрипт для клиента. Имя: ${input.name}. Телефон: ${input.phone}. Email: ${input.email}. Интерес: ${input.interest}. Выезд: ${input.departure}. Канал: ${input.channel}. Комментарий клиента: ${input.message}. Учитывай CRM-контекст ниже, но не выводи его как список задач и не называй задачи. На основе задач, пометок и истории добавь новые этапы разговора: что уточнить, что сказать, какой риск закрыть, к чему вести клиента. Если старый сценарий уже не подходит, замени его полностью.\n\nCRM-контекст для анализа, не для прямого вывода:\n${crmContext || 'Дополнительного контекста пока нет.'}\n\nВерни только готовый скрипт: цель звонка, открытие, этапы разговора, персональные вопросы, аргументы, фразы для ближайшего действия, обработка риска/возражения, следующий шаг. Не добавляй разделы «текущие задачи», «будущие задачи», «CRM-контекст», «техническая сводка».`,
            },
          ],

...
function fallbackLeadScript(input: {
  ...
  return [
    ...
    input.context ? `Вывод по CRM-контексту: не перечислять технические статусы задач, а использовать их как основание для следующего шага.\n${input.context}` : '',
  ].join('\n\n')
}
      ...
      const response = await fetch('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({
          ...
          input: [
            ...
            {
              role: 'user',
              content: `Составь или полностью замени персональный скрипт для клиента. Имя: ${input.name}. Телефон: ${input.phone}. Email: ${input.email}. Интерес: ${input.interest}. Выезд: ${input.departure}. Канал: ${input.channel}. Комментарий клиента: ${input.message}. Обязательно учитывай весь CRM-контекст ниже: задачи, пометки, комментарии, историю действий. Не перечисляй статусы задач и не вставляй техническую сводку. Используй задачи только как источник выводов: что уже сделано, что мешает, что нужно сказать дальше и каким должен быть следующий шаг.\n\nCRM-контекст:\n${crmContext || 'Дополнительного контекста пока нет.'}\n\nВерни структуру: короткая цель звонка, обновлённое открытие, 5 персональных вопросов, аргументы под ситуацию клиента, что сказать по ближайшему действию, обработка риска/возражения, следующий шаг.`,
            },
          ],
